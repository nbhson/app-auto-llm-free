...
| **Dashboard (6 Trang)** | **Dashboard**: 4 stat cards + token overview (biểu đồ chuyển sang Usage)<br>**Providers**: phân trang 25/50, sticky header, filter `hasKey` (mặc định TẮT), `Sync Live Now` + **auto boot-sync** (provider mới → `★ NEW` tím + `addedAt`), `Get Key ↗`, live health, **fetch 1 lần khi reload**<br>**Models**: phân trang 25/50, Filters dropdown (`hasKey` TẮT, `Hide 404`/`credits`/`invalid` BẬT), `Check Live (n)`, `Sync Live Now` + **auto sync** (1 lần khi reload), `Refresh`, `Used/Limit`, strikethrough persisted qua `POST /api/models/health/mark`<br>**Keys**: `fgk-...` CRUD + Generator + Quick Test<br>**Usage**: Topology provider (App giữa + line xanh/tím animated), **auto highlight provider mới nhất** (`newestProviders` tím), tokens/requests theo provider, pie status, SSE live, fetch-all pagination, **fetch 1 lần khi reload**<br>**Logs**: bảng request + SSE stream (AbortController an toàn) |
