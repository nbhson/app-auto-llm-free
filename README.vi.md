...
| **Dashboard (6 Trang)** | **Dashboard**: 4 stat cards + token overview (biểu đồ chuyển sang Usage)<br>**Providers**: phân trang 25/50, sticky header, filter `hasKey` (mặc định TẮT), `Sync Live Now` + **auto boot-sync** (provider mới → `★ NEW` tím + `addedAt`), `Get Key ↗`, live health, poll 8s + `/api/sync/status`<br>**Models**: phân trang 25/50, Filters dropdown (`hasKey` TẮT, `Hide 404`/`credits`/`invalid` BẬT), `Check Live (n)`, `Sync Live Now` + **auto sync** (live `generated_at` đổi → auto reload), `Refresh`, `Used/Limit`, strikethrough persisted qua `POST /api/models/health/mark`<br>**Keys**: `fgk-...` CRUD + Generator + Quick Test<br>**Usage**: Topology provider (App giữa + line xanh/tím animated), **auto highlight provider mới nhất** (`newestProviders` tím), tokens/requests theo provider, pie status, SSE live, fetch-all pagination, poll sync<br>**Logs**: bảng request + SSE stream (AbortController an toàn) |
...
> **⚠️ Sau khi sửa `.env` phải kill gateway cũ rồi chạy lại** — gateway chỉ đọc `.env` lúc boot (`config.ts:22`), `tsx watch` **không** tự reload `.env`. Sau khi restart, **auto boot-sync** (`jobs/boot-sync.ts`) tự phát hiện provider mới (qua `data/.provider-fingerprint.json`), tự `syncLiveModels` + `verify` và 3 trang **Providers / Models / Usage** tự poll cập nhật — không cần bấm `Sync Live`. Xem **Kill process cũ → restart** bên dưới.
