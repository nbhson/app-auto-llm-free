...
* **Dashboard**: Vite + React (recharts), 5 routes `Dashboard→Providers→Models→Keys→Logs` (header 2 hàng: `30 providers • 316 free` + Master hàng 1, nav giữa hàng 2), `Dashboard` 4 cards + 3 charts + tokens, `Models` **filter bar**: `q` + `provider` (filter by provider, datalist 20, debounce 400ms) + `verified` + pill `hasKey`/`hide404`/`hidePayment`, hàng 2 `Check Live`/`Sync Live`/`Refresh`, pagination 25/50 sticky bottom + checkbox (disable 404/410, vừa Check Live 404 vẫn hiện) + `Used/Limit` + strikethrough persist, `Providers` pagination 25/50 + `Get Key ↗` + health, `Keys` Generator + CRUD `fgk-...`, `Logs` charts + SSE.
