...
- **`/models`** — Live `882 free` / freellms `316 free`, **1 hàng filter**: `Filter id` + `Filter provider` + `Verified` + **Filters** dropdown (ngay cạnh Verified, 4 toggles: `hasKey` xanh `#dcfce7` + `Ẩn 404` đỏ `#fee2e2` + `Hide credits` amber + `Hide invalid ID` slate — đều **mặc định checked**) + bên phải **3 nút** `Check Live (n)` (primary xanh + badge) — `Sync Live Now` (xanh lá, freeOnly) — `Refresh` (trung tính) (đã dời từ hàng 2 lên, nhờ center rộng); checkbox per row + header chọn tất cả (disable khi 404/410/invalid/payment strikethrough `#dc2626`/`#e5e7eb`), cột **Used / Limit** (đếm từ logs vs `Up to 40 RPM`), **Live** (`✅ usable 123ms`/`no-key`/`unusable 410`). **Sticky bottom pagination** `Page X/Y` + `LOV 25/50` (đã dời khỏi top filter). 404 persisted trong `data/model-health.json` + localStorage, ẩn khi hide404 checked; `Hide invalid ID` lọc model thiếu `/` hoặc chứa space/ký tự lạ (alias `auto` loại trừ).
