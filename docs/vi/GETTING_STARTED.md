...
Mở Dashboard: **http://localhost:3000** — header **2 hàng** `max-w-[1440px]` rộng hơn (`lg:px-6`): hàng 1 trái `⚡ Free LLM Gateway • 30 providers • 316 free` + `● online`, phải `VI/EN` + **Master input chỉnh sửa** (toggle password/text + Show/Copy, tự điền từ `GET /api/bootstrap` lần đầu, luôn admin) — sửa trực tiếp trên header nếu cần; hàng 2 nav `Dashboard→Providers→Models→Keys→Logs` căn giữa. `ENCRYPTION_KEY` nội bộ, tự sinh. Để rotate, mở **Keys → Key Generator** (mặc định collapsed).

> Muốn tự đặt key: sửa `MASTER_KEY`/`ENCRYPTION_KEY` trong `.env` trước khi `compose up`. Clone mới `data/` trống (chỉ `.gitkeep` — xem `b930e6d`); bấm **Sync Live Now** ở `/providers` hoặc `/models` (`POST /api/models/live/sync {freeOnly:true}`) để tạo `data/live-models.json` sau khi thêm provider keys thật.
...
- **Header 2 hàng** `max-w-[1440px]` rộng hơn (`lg:px-6`): Hàng 1 trái `⚡ Free LLM Gateway` + `● online/offline` + `30 providers • 316 free`, phải `VI/EN` + **Master input chỉnh sửa** (toggle password/text + Show/Copy, tự điền từ `GET /api/bootstrap` nếu localStorage placeholder, re-bootstrap khi 401) — chỉnh trực tiếp; Hàng 2 nav 5 tabs `Dashboard→Providers→Models→Keys→Logs` căn giữa (`alignSelf: center`, `borderRadius:10`). Language persist `localStorage lang`, dịch nav + Models/Providers (`lib/i18n.tsx`). Bootstrap tắt mặc định (`EXPOSE_BOOTSTRAP=0`); bật local `EXPOSE_BOOTSTRAP=1` để tự bind lần đầu (`app.ts:23`). Clone mới `data/` trống (`.gitkeep` only) — dùng **Sync Live Now** để nạp.
- **`/` Dashboard** — 4 cards: Providers 41, Verify 314/316, Requests, **Tokens** (all-time + last 100 prompt/completion). Ngay dưới là **Quick Guideline** 3 bước (gradient `slate-900`, nút `Đi tới Keys`): `1 Auto MASTER_KEY` → `2 Create fgk-...` → `3-4 Copy & Quick Test`. Tiếp là 3 charts: **Requests by Provider**, **Latency**, **Verify Pie** + Recent Logs (5) + **Tokens by Provider** bar. **Gateway Health & Stats Detail** đã dời ra **sidebar phải** (nút `Gateway Health & Stats Detail` mở drawer `w-[520px]`, backdrop, Copy JSON).
- **`/providers`** — 41 dòng, `Free`, `Keys` (`✓ real` xanh khi `hasRealKey` `k.length>20 && !xxx`), **Health** (`online`/`offline`/`no-key` + `breaker`), **Caps**, cột **Get Key ↗** (link thẳng console provider + freellms). Hàng có hasRealKey highlight `background #f0fdf4`, `borderLeft 3px #16a34a`, badge `● has key` xanh lá. Top filter: `q` search (debounce 400ms) + pill `Chỉ hiện đã nhập key` (hasKey, mặc định tắt), **sticky bottom pagination** `‹ Prev / Next ›` + `Page X/Y` + `LOV 25/50`. Nút **Sync Live Now** (cùng `POST /api/models/live/sync` như Models, ghi `data/live-models.json`, sau đó `POST /api/verify`) + **Live Health Check** 41 providers 5s. Sau đổi `.env` phải **restart gateway** mới thấy `hasKey` xanh.
- **`/models`** — Live `882 free` / freellms `316 free`, **1 hàng filter**: `Filter id` + `Filter provider` + `Verified` + **Filters** dropdown (ngay cạnh Verified, 4 toggles: `hasKey` **mặc định tắt** `hasKeyOnly:0` + `Ẩn 404`/`Hide credits`/`Hide invalid ID` mặc định bật) + bên phải **3 nút** `Check Live (n)` (yêu cầu `Filter id` hoặc `provider`, tick chọn trước) / `Sync Live Now` (cùng `POST /api/models/live/sync` với Providers, `freeOnly:true`, ghi `data/live-models.json`) / `Refresh` (xóa `q`/`provider`/`verified`, reset `hasKeyOnly:false` + 3 `hide*` true, không tự bật `hasKey`); checkbox per row + header chọn tất cả (disable khi `isInvalidId`), cột **Used / Limit** (logs vs `Up to 40 RPM`), **Live** `usable/no-key/402/404/410` (`Check` per-row `GET /api/models/health?model=` → `POST /api/models/health/mark {status:"usable",http_status:200}` persist `data/model-health.json` để reload giữ không đỏ, `isRowDisabled` ưu tiên `live usable`/`usage>0` trước `deprecated`). `Hide invalid ID` lọc model thiếu `/` hoặc chứa space/ký tự lạ (alias `auto` loại trừ).
