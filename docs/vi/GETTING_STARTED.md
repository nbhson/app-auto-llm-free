...
Mở Dashboard: **http://localhost:3000** — header **2 hàng** `max-w-[1440px]` rộng hơn (`lg:px-6`): hàng 1 trái `⚡ Free LLM Gateway • 30 providers • 316 free` + `● online`, phải `VI/EN` + **Master read-only** (masked + Show/Copy, tự sinh, luôn có admin) — không cần nhập; hàng 2 nav `Dashboard→Providers→Models→Keys→Logs` căn giữa. `ENCRYPTION_KEY` nội bộ, tự sinh. Để rotate, mở **Keys → Key Generator** (mặc định collapsed).

> Muốn tự đặt key: sửa `MASTER_KEY`/`ENCRYPTION_KEY` trong `.env` trước khi `compose up`.
