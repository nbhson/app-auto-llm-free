...
- **Prod**: configure at least 5 P0 keys (NVIDIA, Groq, Cerebras, Gemini, GitHub) for live sync of 882 free (853 hasKey) every 24h; scheduler auto-calls both verify and syncLiveModels. Remaining providers will stay `unverified_no_key` but still serve with a warning
  - **Dashboard**:
    ...
    - `/providers`: filter `q` 400ms debounce + pill `hasKey` (default OFF) + `hasRealKey` green highlight; sticky bottom `LOV 25/50`; **Manual Refresh** (env-based, cached `providersCache`/`providersSyncCache`, initial fetch if no cache so not empty) + **Sync Live Now** same endpoint as Models, writes `data/live-models.json`
      - `/chat`: 6 allowed models only (`free-llm-gateway/auto`, `kilo-code/kilo-auto/free`, `kilo-code/auto`, `openrouter/auto`, `kiraai/kira-auto`, `agnes-ai/agnes-2.5-flash`), strict selector, **1.8.0 web + robust streaming** — `extractDelta` handles `content`/`text`/`output_text` + `reasoning_content`/`reasoning`/`thinking` + array `[{text}]` + top-level, skips `:` ping/`event:`, buffers split JSON, `reasoningFull` fallback (fixes empty when only thinking), `streamError` + leftover `data:` flush + **non-stream fallback 1×** (`stream:false`), **gateway web_search + web_fetch** (`lib/web-tools.ts`, SSRF guard, HTML extract 12k, cache 3600s, Globe toggle `x-web-tools:1`, loop 3), **truncated detection** (`finish_reason:length` → warning + **▶ Continue** `handleContinue`), default `maxTokens 8192` (was 4096, max 16384) to avoid truncation + allow continue, markdown/code-block with copy, auto-scroll, right context-window with **breakdown clickable → scroll to message** (`highlightedId` ring), upload image/`.md`/`.txt` (50k), paste/drag, system prompt/temperature/maxTokens/stream, Refresh clears session, cache preserved
