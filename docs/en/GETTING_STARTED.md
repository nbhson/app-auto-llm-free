...
Open the Dashboard: **http://localhost:3000** — **2-row header** `max-w-[1440px]` wider center (`lg:px-6`): row 1 left `⚡ Free LLM Gateway • 30 providers • 316 free` + `● online`, right `VI/EN` + **Master read-only** (masked + Show/Copy, auto-generated, always has admin) — no edit needed; row 2 centered nav `Dashboard→Providers→Models→Keys→Logs`. `ENCRYPTION_KEY` is internal — auto-generated. To rotate, open **Keys → Key Generator** (collapsed by default).
...
- **`/models`** — Live `882 free` / freellms `316 free`, **single filter row**: `Filter id` + `Filter provider` + `Verified` + **Filters** dropdown (right next to Verified, 4 toggles: `hasKey` green `#dcfce7` + `Hide 404` red `#fee2e2` + `Hide credits/payment` amber + `Hide invalid ID` slate — all **checked by default**) + **top-right 3 buttons** `Check Live (n)` (primary blue + badge) — `Sync Live Now` (green, freeOnly) — `Refresh` (neutral) (moved up from centered second row, thanks to wider center). Table: checkbox per row + header select-all (disabled when 404/410/invalid/payment strikethrough `#dc2626`/`#e5e7eb` + `line-through`), **Used / Limit** column (counted from logs vs `Up to 40 RPM`), **Live** (`✅ usable 123ms`/`no-key`/`unusable 410`). **Sticky bottom pagination** `Page X/Y` + `LOV 25/50` (moved from top filter). 404 persisted in `data/model-health.json` + localStorage, hidden when hide404 checked; `Hide invalid ID` filters models without `/` or with spaces/illegal chars (alias `auto` excluded).
