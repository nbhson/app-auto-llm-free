...
* **Dashboard**: Vite + React (recharts, i18n `lib/i18n.tsx` VI/EN), 5 routes `Dashboard→Providers→Models→Keys→Logs` (**2-row header**: row 1 left `⚡ Free LLM Gateway` + `● online/offline` + `30 providers • 316 free`, right `VI/EN` selector + **Master** input on same row; row 2 centered nav `Dashboard→Providers→Models→Keys→Logs` via `alignSelf: center` — previously single row with centered nav via grid). `Dashboard` with 4 cards + 3 charts + tokens, `Models` **top filter bar**: search `q` (400ms debounced) + `provider` filter by provider (text input with datalist 20 providers, 400ms debounced, `?provider=` exact) + `verified` select + 2 pill toggles `hasKey` / `hide404` on right, **second row** 3 centered buttons `Check Live (n)` (primary blue + count badge) — `Sync Live Now` (green, freeOnly) — `Refresh` (neutral), **sticky bottom pagination**: `‹ Prev / Next ›` + `Page X/Y` + `LOV 25/50` (moved from top filter bar), table with strikethrough `#dc2626` + disabled checkbox for 404/410 persisted (just-checked 404 keeps visible with strikethrough, hide only applies to persisted `m.health` so `Check Live` result does not instantly disappear), `Providers` with sticky bottom pagination 25/50 + filter + `hasKey` pill + `hasRealKey` highlight, `Keys` with Generator + CRUD `fgk-...`, `Logs` with charts + SSE Live ON (removed duplicate Auto sync 5s, kept only SSE + 2s poll).
