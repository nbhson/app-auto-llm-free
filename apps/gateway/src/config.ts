...
function ensureKeysAutoGenerated() {
  ...
  if (isPlaceholderMaster(process.env.MASTER_KEY)) {
    ...
    changed = true;
    // Only log full key in development to avoid leaking in prod logs
    if ((process.env.NODE_ENV || "development") === "development") {
      console.log(`[config] Auto-generated MASTER_KEY=${gen} (no valid .env value found)`);
    } else {
      console.log(`[config] Auto-generated MASTER_KEY=${gen.slice(0, 12)}... (redacted in production)`);
    }
  }
...
export const config = {
  ...
  providerKeys: {
    ...
    kira: parseKeys(process.env.KIRAAI_API_KEYS || process.env.KIRA_API_KEYS),
