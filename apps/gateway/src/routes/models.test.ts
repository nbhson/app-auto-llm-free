...
describe("models route", () => {
  ...
  it("limit=50 paginates, unknown limit falls back to 25", async () => {
    const r50: any = await (await modelsRoute.request("/?limit=50")).json();
    expect(r50.data).toHaveLength(50);
    expect(r50.pagination.limit).toBe(50);
    const rBad: any = await (await modelsRoute.request("/?limit=30")).json();
  ...
  it("page=2 differs from page=1, clamps beyond last page", async () => {
    const p1: any = await (await modelsRoute.request("/")).json();
    const p2: any = await (await modelsRoute.request("/?page=2")).json();
    expect(p2.pagination.page).toBe(2);
    expect(p2.data[0].id).not.toBe(p1.data[0].id);
    const far: any = await (await modelsRoute.request("/?page=9999")).json();
  ...
  it("provider filter narrows to that provider", async () => {
    const data: any = await (await modelsRoute.request("/?provider=groq&limit=50")).json();
  ...
  it("hasKey filter returns 200 with echo flag (env-agnostic shape)", async () => {
    const data: any = await (await modelsRoute.request("/?hasKey=1")).json();
    expect(data.filters.hasKey).toBe(true);
    expect(Array.isArray(data.data)).toBe(true);
  });

  it("GET /:id echoes unknown model, finds freellms model", async () => {
    const unknown: any = await (await modelsRoute.request("/some-unknown-model")).json();
